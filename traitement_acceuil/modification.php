                ...
                <div id="content">
                    ...
                    <div style="margin-left: 20px; width: 100%; ">
                        <div class="login-box">
                            <div class="form">
                                <u>
                                    <h2>Modification libellé </h2>
                                </u>
                                <br>
                                <?php
                                include('../bdd/connexion_bdd.php');
                                $liste = "SELECT * FROM acceuil WHERE id_acceuil  = '$id_acceuil' ";

                                $sql = mysqli_query($conn, $liste);

                                while ($row = mysqli_fetch_assoc($sql)) {
                                    # code...
                                    echo '<form class="login-form" action="traitement_modifier.php" method="POST">
                                <div class="input-group">
                                    <input type="text" name="libelle_1" value="' . $row['libelle_1'] . '" />
                                    <input type="text" name="libelle_2" value="' . $row['libelle_2'] . '" />
                                </div>
                                <div class="input-group">
                                    <input type="text" name="libelle_3" value="' . $row['libelle_3'] . '" />
                                    <input type="text" name="libelle_4" value="' . $row['libelle_4'] . '" />
                                </div>
                                <input type="hidden" value="' . $row['id_acceuil'] . '" name="id_acceuil">
